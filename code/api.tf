...
locals {
  portal_backend_1_app_settings = {
    ...
    CGN_APIM_PRODUCTID     = var.io_apim_v2_productid != null ? var.io_apim_v2_productid : (var.io_apim_v2_productid != null ? var.io_apim_v2_productid : data.azurerm_api_management_product.cgn_onbording_portal_v2.id)
...
module "portal_backend_1" {
  ...
  allowed_subnets = [data.azurerm_subnet.subnet_apim_v2.id]
  allowed_ips     = []
...
module "ade_aa_mock" {
  ...
  allowed_subnets = [data.azurerm_subnet.subnet_apim_v2.id, module.subnet_spid_login.id, module.subnet_api.id]
  allowed_ips     = []
...
module "app_operator_search" {
  ...
  allowed_subnets = concat(
    [data.azurerm_subnet.subnet_apim_v2.id, ],

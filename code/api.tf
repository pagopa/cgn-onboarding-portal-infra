...
module "spid_login" {
  ...
  app_settings = merge({
    ...
    METADATA_PUBLIC_CERT  = var.agid_spid_pem_name != null ? data.azurerm_key_vault_secret.agid_spid_pem[0].value : tls_self_signed_cert.spid_self.cert_pem
    METADATA_PRIVATE_CERT = var.agid_spid_key_name != null ? data.azurerm_key_vault_secret.agid_spid_key[0].value : tls_private_key.spid.private_key_pem

...
module "spid_login" {
  ...
  app_settings = merge({
    ...
    METADATA_PUBLIC_CERT  = var.spid_certificate_name != null ? data.azurerm_key_vault_certificate_data.spid_cert[0].pem : tls_self_signed_cert.spid_self.cert_pem
    METADATA_PRIVATE_CERT = var.spid_certificate_name != null ? data.azurerm_key_vault_certificate_data.spid_cert[0].key : tls_private_key.spid.private_key_pem
